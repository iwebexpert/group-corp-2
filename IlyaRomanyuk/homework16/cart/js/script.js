class Backet {
  ...
  /*методы работы с апи db.json*/

  //Удаление
  methodDELETE(element) {
    return fetch(`/cart/${element['id']}`, { method: 'DELETE' })
  }
  ...
  // Получение
  methodGET() {
    return fetch(`/cart`, { method: 'GET' });
  }

  //Редактирование
  methodPATCHINC(index) {
    return fetch(`/cart/${index['id']}`, {
      method: 'PATCH',
      body: JSON.stringify({ quantity: index.quantity + 1 }),
      headers: {
        'Content-type': 'application/json',
      },
    })
  ...
  async increment(index, event) {
    let response = await this.methodPATCHINC(this.basket[index]);
    if (response.ok) {
  ...
  async decrement(index, event) {
    if (this.basket[index].quantity - 1 > 0) {
      let response = await this.methodPATCHDEC(this.basket[index])
      if (response.ok) {
        this.basket[index].quantity--;
  ...
  async deleteProductInCart(e, element) {
    document.querySelector("#cart").removeChild(e.target.parentElement);
    let response = await this.methodDELETE(this.basket[element]);
    if (response.ok) {
  ...
  async addToCart(element) {
    ...
    if (this.basket[goodsId]) {
      let response = await this.methodPATCHINC(this.basket[goodsId]);
      if (response.ok) {
      ...
      let response = await this.methodPOST(element);
      if (response.ok) {
        let response = await this.methodGET();
        let data = await response.json();
        this.basket[goodsId] = data.find(element => element.id == goodsId);
      }
      this.renderCart(`${this.countBasketPrice()} ${this.quantityInCart()}`);
      this.renderProductInCart();
    }
  ...
  async clearCart() {
    for (let key in this.basket) {
      let response = await this.methodDELETE(this.basket[key]);
...
class Order {
  ...
  onSuccess() {
    ...
    closeBtn.addEventListener("click", async e => {
      for (let key in this.cart.basket) {
        let response = await fetch(`/cart/${this.cart.basket[key].id}`, { method: 'DELETE' });
        console.log(response)
      }
      location.reload()
    });
