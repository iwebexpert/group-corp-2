...
type ChatItemProps = {
    ...
    image: string,
    title: string,
    deleteMessageAction: (chatsId: number, id: string) => void
}

export const ChatItem: React.FC<ChatItemProps> = ({ id, author, message, chatsId, image, title, deleteMessageAction }) => {
    let style = '';
    if (title !== author) {
        style = '--right';
    }

    const deleteMess = (id: string, chatsId: number) => {
