...
export const chatsLoadAction = () => {
    return async (dispatch) => {
        try {
            ...
            const chats = await fetch('http://localhost:3000/chats?_embed=messages').then(chats => chats.json());
            const friends = await fetch('http://localhost:3000/friends').then(friends => friends.json());
...
export const chatsMessageSendAction = (message, chats, answerCount) => {
    return async (dispatch) => {
        try {
            const newMsg = await fetch('http://localhost:3000/messages', {
                method: 'POST',
                body: JSON.stringify({
                    text: message.text,
                    type: message.type,
            ...
            const changeAnswerCount = await fetch('http://localhost:3000/chats/' + message.chatId, {
                method: 'PATCH',
                body: JSON.stringify({
                    answerCount: answerCount
                }),
...
export const deleteFriendListDialogAction = (friendsList, idToDelete) => {
    return async (dispatch) => {
        try {
            const result = await fetch('http://localhost:3000/friends/' + idToDelete, {
                method: 'DELETE',
                headers: {
                    'Content-type': 'application/json',
                }});
...
export const messageFireAction = (chatId) => {
    return async (dispatch) => {
        try {
            const result = await fetch('http://localhost:3000/chats/' + chatId.chatId, {
                method: 'PATCH',
                body: JSON.stringify({
                    fire: true
                }),
