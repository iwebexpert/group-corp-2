class Validator {
    ...
    initBasePatterns() {
        this.patterns[Validator.basePatternsName.name] = /^\S*[A-Za-z\s-]+\S*$/;
        ...
        this.patterns[Validator.basePatternsName.house] = /^\S*[A-Za-z0-9\s-]+\S*$/;
        this.patterns[Validator.basePatternsName.street] = /^\S*[A-Za-z\d\s-]+\S*$/;
        this.patterns[Validator.basePatternsName.city] = /^\S*[A-Za-z\s-]+\S*$/;
        this.patterns[Validator.basePatternsName.comment] = /^[\w\s.,!?;:-]*$/;
    ...
    constructor(initializatorDOM){
        ...
        this.formsValid = {
            contacts: {
                city: {
                    isValid: false,
                    errorMessage: 'Требуется: латиница, -, пробел, без пробелов по краям'
                },
                street: {
                    isValid: false,
                    errorMessage: 'Требуется: латиница, цифрыб -, пробел, без пробелов по краям'
                },
                house: {
                    isValid: false,
                    errorMessage: 'Требуется: латиница, цифры, -, пробел, без пробелов по краям'
                },
                phone: {
                    isValid: false,
            ...
            more: {
                comment: {
                    isValid: false,
                    errorMessage: 'Требуется: латиница, -, пробел'
                }
            }
        };
    }
        ...
        saveBtn.addEventListener('click', (e) => {
            switch (e.target.id) {
                case 'addressSaveButton': {
                    const {city, street, house, phone, email, name} = this.addressWindow.elements;
                    this.validateInput([city, street, house, phone, email, name], 'contacts');
                    break;
                }
                case 'commentsSaveButton': {
                    const {comment} = this.commentsWindow.elements;
                    this.validateInput([comment], 'more');
                    break;
                }
