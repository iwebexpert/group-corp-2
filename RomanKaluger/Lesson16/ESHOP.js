class DatabaseManager {
    ...
    async requestQueueDelay() {
        return new Promise(resolve => {
            setTimeout(() => {
                setTimeout(() => this.requestQueueCounter -= 1,this.delayTime);
                resolve(true);
            }, this.delayTime * this.requestQueueCounter);
            this.requestQueueCounter += 1;
        })
    }
    async get(){
        return await this.getMethod('', (xhr) => JSON.parse(xhr.responseText), () => null);
    }
    async postMethod(params, data) {
...
class StatisticsRecord {
    ...
    constructor (type, id, details) {
        if (!(type in StatisticsRecord.types)) {
...
class BasketStatisticsDatabaseManager extends DatabaseManager{
    constructor(){
        super('http://localhost:3000/basketStats');
    }
    async postRecord(record) {
        if (!(record instanceof StatisticsRecord)){
            throw new Error('некорректные данные');
        }
        await this.postMethod('', record);
    }
    async getRecords() {
        return await this.getMethod('',(xhr) => JSON.parse(xhr.responseText), () => {throw new Error('проблема в BasketStatisticsDatabaseManager')});
    }
    async getRecordsByType(type) {
        return await this.getMethod(`/?type=${type}`,(xhr) => JSON.parse(xhr.responseText), () => {throw new Error('проблема в BasketStatisticsDatabaseManager')});
    }
}
class BasketDatabaseManager extends DatabaseManager{
    constructor(){
        super('http://localhost:3000/basket');
    }
    _checkIfItem (item){
        if (!(item instanceof ProductGroupItem)) {
            throw new Error('некорректные данные');
        }
    }
    async getBasketItems() {
        return await this.getMethod('',(xhr) => JSON.parse(xhr.responseText), () => {throw new Error('проблема в BasketDatabaseManager')});
    }
    async postBasketItem(item) {
        this._checkIfItem(item);
        return await this.postMethod('/', item);
    }
    async patchBasketItem(item) {
        return await this.patchMethod(`/${item.id}`, item);
    }
    async putBasketItems(items) {
        return await this.putMethod(``, items);
    }
    async deleteBasketItem(item) {
        this._checkIfItem(item);
...
class BasketStatistics {
    constructor(){
        ...
        this.db = new BasketStatisticsDatabaseManager();
    }
    init(toBasketStatisticsBtn){
        this.toBasketStatisticsBtn = toBasketStatisticsBtn;
    }
    initDom(){
        const basketStatisticsBoard = document.createElement('div');
        basketStatisticsBoard.classList.add('BasketStatisticsBoard');
        const label = document.createElement('div');
        label.innerHTML = 'Статистика корзины';
        label.classList.add('dashBoardMainLabel');
        basketStatisticsBoard.append(label);
        return basketStatisticsBoard;
    }

    async updateCharts() {
        const recordsArr = await this.db.getRecords();
        if (this.dataTable) {
            this.dataTable.destroy();
            $('#tableHtmlBasketStats').get()[0].remove();
        }
        const tableHTML = `<table id="tableHtmlBasketStats" class="tableHtmlBasketStats display">
        <thead>
            <tr>        <th>Id</th>             <th>Type</th>           <th>Product</th>                 <th>Amount</th>                <th>Date</th>           </tr>
        </thead>
        <tbody>
        ${recordsArr.reduce((acc,rec)=>acc +
            `<tr>      <td>${rec.id}</td>      <td>${rec.type}</td>    <td>${rec.details.product.name}</td>     <td>${rec.details.amountDiff}</td>     <td>${rec.details.date}</td>    </tr>`
        ,'')}  
        </tbody>
        <tfoot>
            <tr>        <th>Id</th>             <th>Type</th>           <th>Product</th>                 <th>Amount</th>                <th>Date</th>           </tr>
       ...
       this.basketStatisticsBoard.insertAdjacentHTML('beforeend', tableHTML);
       this.dataTable = $('#tableHtmlBasketStats').DataTable({
           "lengthMenu": [ 10, 15, 20]
       });
    }
    render() {
        return this.basketStatisticsBoard;
    }
}
class Sorter{
    ...
    initHandlers(){
        ...
        this.button.addEventListener('click', () => {
            if (this.isPageOpen){
                ...
                if (this.button.firstChild.tagName === 'IMG'){
...
class ToBasketStatisticsButton extends ChangePageButton{
    constructor() {
        super();
    ...
    initDom (){
        ...
        button.classList.add('toBasketStatisticsButton');
        button.innerHTML = 'Статистика корзины';
        return button;
    }
}
class Chart {
...
class ToDashBoardButton extends ChangePageButton{
    constructor() {
        super();
        this.button = this.initDom();
    }
...
class ShopInitializator{
    ...
    async init(){
        ...
        await this.basket.synchronizeItemsWithDB();
        this.initializatorDOM.initializeShop(this.catalog, this.basket);
    }
}
class SearchFilters extends Sorter{
    constructor(initializatorDOM) {
        super();
        this.initDOM();
        this.initializatorDOM = initializatorDOM;
...
class InitializatorDOM {
    ...
    initializeShop(catalog, basket) {
        ...
        tabToggleButton.onmousedown = (async () => {
            await basket.synchronizeItemsWithDB();
        ...
        this.basketStatistics = new BasketStatistics();

        this.toDashBoardButton = new ToDashBoardButton();
        this.toBasketStatisticsButton = new ToBasketStatisticsButton();

        this.toDashBoardButton.init(this.container, this.dashBoard);
        this.toBasketStatisticsButton.init(this.container,  this.basketStatistics);
        ...
        this.basketView.basketStatusWindow, this.toDashBoardButton.render(), this.toBasketStatisticsButton.render());
    ...
    createCatalogCard(productGroupItem, mode) {
        ...
        productDescription.innerHTML = `${productGroupItem.product.description}<br><br>/${Object.keys(ProductToEat.productSizes).reduce((acc, val)=> acc + `${Math.round(ProductToEat.getSizeMultiplier({size:ProductToEat.productSizes[val]}) * productGroupItem.product.getBasePrice())}/`,'')}р без добавок`;
        ...
        const onAmountPlus = () => {
            ...
            if (mode === 'BASKET') {
                basket.addItem(productGroupItem.product, 1, Basket.basketAddItemsModes.MERGE);
        ...
        const onAmountMinus = () => {
            ...
            if (mode === 'BASKET') {
                if (productCount.innerHTML === '0') {
                    ...
                    basket.decreaseItem(productGroupItem, 1);
                }
                const totalInfoPanel = document.querySelector(`.totalInfoPanel`);
                totalInfoPanel.remove();
        ...
        if (mode === 'CATALOG') {
            ...
            toBasketBtn.onclick = () => {
                if (productGroupItem.amount > 0) {
                    ...
                    if (basket.findItem(productGroupItem.product.name)){
                        basket.addItem(productGroupItem.product, productGroupItem.amount, Basket.basketAddItemsModes.MERGE);
                    } else {
                        basket.addItem(productGroupItem.product, productGroupItem.amount, Basket.basketAddItemsModes.ADD_OR_RESET);
                    }
                    productGroupItem.amount = 0;
                }
            };
...
class ProductToEat extends Product{
    ...
    toJSON(){
        return {
            ...
            imgURLsArr: this.imgURLsArr,
            ingredientsArr: this._ingredientsArr,
            size: this.size,
...
class Basket {
    ...
    constructor() {
        this.basketDatabaseManager = new BasketDatabaseManager();
        this.basketStatisticsDatabaseManager = new BasketStatisticsDatabaseManager();
    }
    convertServerItemsToClasses(basketItems) {
        basketItems.forEach(x => {
            const converter = new Converter();
            const ingredients = x._product.ingredientsArr.map(i => converter.convertProductNameToProduct(i.name));
            const product = new ProductToEat(x._product.name, +x._product.price, x._product.description, x._product.features, x._product.imgURLsArr,ingredients, x._product.size, +x._product.baseCalories);
            this._addItemNoServer(product, x._amount, 'ADD_OR_RESET');
        });
    }
    async synchronizeItemsWithDB() {
        const basketItems = await this.basketDatabaseManager.getBasketItems();
        this._items = [];
        this.convertServerItemsToClasses(basketItems);
    }
    findItem(name) {
        return this._items.find(x => x.product.name === name);
    }
    _addItemNoServer(product, amount, mode) {
        ...
        let resultProductItem = null;
        switch (mode) {
            case Basket.basketAddItemsModes.ADD_OR_RESET: {
                if (found) {
                    found.amount = amount;
                    resultProductItem = found;
                } else {
                    resultProductItem = new ProductGroupItem(product, amount);
                    this._items.push(resultProductItem);
                }
                break;
            }
            case Basket.basketAddItemsModes.MERGE: {
                if (found) {
                    ...
                    resultProductItem = this._addItemNoServer(product, amount, Basket.basketAddItemsModes.ADD_OR_RESET);
                }
                break;
            }
            default:
                throw new Error('Видимо, все плохо');
        }
        return resultProductItem;
    }
    decreaseItem(item, decreaseAmount) {
        if (item.amount >=decreaseAmount){
            this.addItem(item.product, -decreaseAmount, Basket.basketAddItemsModes.MERGE);
        }
        else {
            throw new Error('слишком большое число удаления');
        }
    }
    addItem(product, amount, mode) { // true если добавили успешно
        const addResultItem = this._addItemNoServer(product, amount, mode);
        if (addResultItem){
            const statsType = mode === Basket.basketAddItemsModes.ADD_OR_RESET ? StatisticsRecord.types.ADD_ITEM : StatisticsRecord.types.CHANGE_AMOUNT_ITEM;
            const statsRecord = new StatisticsRecord(statsType, Date.now() ,{date: moment().format('MMMM Do YYYY, h:mm:ss a'), product: addResultItem.product, amountDiff: amount});
            switch (mode) {
                case Basket.basketAddItemsModes.ADD_OR_RESET: {
                    this.basketDatabaseManager.postBasketItem(addResultItem);
                    break;
                }
                case Basket.basketAddItemsModes.MERGE: {
                    this.basketDatabaseManager.patchBasketItem({
                        _amount: addResultItem.amount,
                        id: addResultItem.id
                    });
                    break;
                }
            }
            this.planPostRecord(statsRecord);
        }
        return addResultItem;
    }
    planPostRecord(statsRecord){
        setTimeout(() => this.basketStatisticsDatabaseManager.postRecord(statsRecord), 0);
    }
    removeItem(product) {
        const itemToRemove =  this._items.find(val => val.product === product);
        this._items = this._items.filter(val => val !== itemToRemove);
        this.basketDatabaseManager.deleteBasketItem(itemToRemove);
        const statsRecord = new StatisticsRecord(StatisticsRecord.types.REMOVE_ITEM, Date.now() ,{date: moment().format('MMMM Do YYYY, h:mm:ss a'), product: itemToRemove.product, amountDiff: '-'});
        this.planPostRecord(statsRecord);
    }

    clearBasket() {
        this.items.forEach(x => {
            this.basketDatabaseManager.deleteBasketItem(x);
            const statsRecord = new StatisticsRecord(StatisticsRecord.types.REMOVE_ITEM, Date.now() ,{date: moment().format('MMMM Do YYYY, h:mm:ss a'), product: x.product, amountDiff: '-'});
            this.planPostRecord(statsRecord);
        });
        this._items = [];
    }
