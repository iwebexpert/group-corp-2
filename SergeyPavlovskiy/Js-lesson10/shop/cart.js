...
class Cart {
  ...
  updateGoodsList() {
    let itemsSet = Array.from(new Set(this.items));
    let arrOffObj = [];
    for (let i = 0; i < itemsSet.length ; i++) {
      arrOffObj.push({name: '', prices: 0});
    };
    for (let i = 0; i <itemsSet.length; i++) {
      for (let j  = 0; j < this.items.length; j++) {
        if (itemsSet[i] === this.items[j]) {
          arrOffObj[i].name = this.items[j];
          arrOffObj[i].price  += this.cost[j];
        };
      };
    };

    for (let i = 0; i < arrOffObj.length; i++){
      let goodsItem = document.createElement('p');
      goodsItem.textContent = `${arrOffObj[i].name} ${arrOffObj[i].price} RUB`;
