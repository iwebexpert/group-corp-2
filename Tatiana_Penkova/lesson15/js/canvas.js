class Stat {
    ...
    drawMonthSales() {
        ...
        let totalValue = 0;
        let colorIndex = 0;
        for (let values in this.options.data) {
            let value = this.options.data[values];
            totalValue += value;
        }
        ...
        for (let values in this.options.data) {
            ...
            if (this.options.doughnutHoleSize) {
                ...
                let SlabelX = this.canvas.width / 2 + (offset + pieRadius / 2) * Math.cos(startAngle + sliceAngle / 2);
    ...
    drawTotalSales() {
        ...
        // Рисование графика
        for (let i = 0; i < this.options.data.length; i++) {
            let dp = this.options.data[i].sales;
