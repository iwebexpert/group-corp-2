...
class objWithBaseFunc {
    ...
    createButtonsCountItem(content, listener) {
        const elem = this.createHTMLElem('button', ['btn', 'btn-outline-dark', 'btn__changeCount'], { textContent: content });
...
class Catalog extends objWithBaseFunc {
    ...
    renderBasketItems(boxToBuy) {
        ...
        const basketItems = this.createHTMLElem('div', ['row', 'row-cols-1', 'row-cols-md-3']);
        ...
        boxToBuy.getAll().map((item) => {
            const basketItem = this.createHTMLElem('div', ['col', 'mb-4'], { id: item.id });
            const basketCard = this.createHTMLElem('div', ['card', 'cards__card']);
            basketCard.insertAdjacentHTML("afterbegin", this.paintItem(item));
            const addDelContainer = this.createHTMLElem('div', ['btns__changeCount', 'd-flex']);
            const input = this.createHTMLElem('input', ['basketItem__input'], { type: 'text', value: item.count });
            ...
            const btnDelete = this.createHTMLElem('div', ['btn__delete'], { id: item.id });
    ...
    renderProducts = (product) => {
        ...
        const catalogItemBox = this.createHTMLElem('div', ['col', 'mb-4', 'catalog__product']);
        const catalogCard = this.createHTMLElem('div', ['card', 'h-100', 'cards__card']);
        ...
        const btnItem = this.createHTMLElem('div', ['btn__buy'], { id: product.id });
        btnItem.addEventListener('click', btnCatalogItem);
        const btnItemContainer = this.createHTMLElem('div', ['btnItem-container']);
        const addDelContainer = this.createHTMLElem('div', ['btns__changeCount', 'd-flex', 'justify-content-center']);
        const input = this.createHTMLElem('input', ['basketItem__input'], { type: 'text', value: product.count });
...
function createAddress() {
    ...
    function renderInput(label, id) {
        ...
        inputBox.insertAdjacentHTML('afterbegin', `<label for=${id}>${label}</label><input class='form-control' name=${id} id=${id}></input>`);
    ...
    textareaComment.insertAdjacentHTML('beforeend', '<label class="taxtareaLabel" for="comment__textarea">Комментарий</label><textarea class="form-control" id="comment__textarea" rows="10" cols="45" name="text"></textarea>');
...
makeGETRequest('/catalogProducts')
    .then((catalogProduct) => {
        catalogProduct.forEach(item => baseWithCatalogProduct.setCatalogProduct(item));
        mainFunc();
    })
    .catch((err) => console.log(err));
// console.log(baseWithCatalogProduct.catalogBox)
